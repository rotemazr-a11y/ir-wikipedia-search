...
class IndexBuilder:
    ...
    def add_document(self,
        ...
        # Process title
        if title:
            title_tokens = tokenize_and_process(title, remove_stops=True)
            if title_tokens:  # Only add if there are tokens after processing
                self.title_docs[doc_id] = title_tokens

        # Process body
        if body:
            body_tokens = tokenize_and_process(body, remove_stops=True)
        ...
        if anchors:
            ...
            anchor_tokens = tokenize_and_process(all_anchor_text, remove_stops=True)
